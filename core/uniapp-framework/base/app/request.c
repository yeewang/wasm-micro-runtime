...
static bool
register_url_handler(const char *url,
...
void
api_send_request(request_t *request,
                 response_handler_f response_handler,
                 void *user_data)
{
    int size;
    char *buffer;
    ...
    if (trans == g_transactions) {
        ...
        if (g_trans_timer == NULL) {
            g_trans_timer =
              api_timer_create(TRANSACTION_TIMEOUT_MS, false, true,
...
on_response(char *buffer, int size)
{
    ...
    if (trans == g_transactions) {
        if (trans->next != NULL) {
            unsigned int elpased_ms, ms_to_expiry,
              now = wasm_get_sys_tick_ms();
